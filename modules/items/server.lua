			...
			image = %q,
				...
				for _, item in pairs(dump) do
					if not ItemList[item.name] then
						...
						---@todo cry
						local itemStr = itemFormat:format(item.name, item.label, item.weight, item.stack, item.close, item.description or 'nil', item.hunger or 'nil', item.thirst or 'nil',item.pee or 'nil', item.poo or 'nil', item.stress or 'nil', item.image or 'box.png')
						-- temporary solution for nil values
						itemStr = itemStr:gsub('[%s]-[%w]+ = "?nil"?,?', '')
