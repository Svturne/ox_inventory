...
local trash = {
	{description = 'Un vieux journal enroulé.', weight = 200, image = 'trash_newspaper'},
	{description = 'Une boite de burger jeté.', weight = 50, image = 'trash_burgershot'},
	...
	{description = 'Une cartouche de cigarettes vide.', weight = 10, image = 'trash_fags'},
	{description = 'Une culotte légèrement usée.', weight = 20, image = 'panties'},
	...
	{description = 'Un sac de chips vide.', weight = 5, image = 'trash_chips'},
	{description = 'Un morceau de pomme en décomposition', weight = 5, image = 'trash_apple'},
}
			...
			image = %q,
				...
				for _, item in pairs(dump) do
					if not ItemList[item.name] then
						...
						---@todo cry
						local itemStr = itemFormat:format(item.name, item.label, item.weight, item.stack, item.close, item.description or 'nil', item.hunger or 'nil', item.thirst or 'nil',item.pee or 'nil', item.poo or 'nil', item.maladie or 'nil', item.stress or 'nil', item.image or 'nil')
						-- temporary solution for nil values
						itemStr = itemStr:gsub('[%s]-[%w]+ = "?nil"?,?', '')
